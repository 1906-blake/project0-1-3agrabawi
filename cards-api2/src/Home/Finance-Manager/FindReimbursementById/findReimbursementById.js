...
async function ReimbursementsById(){
    sessionStorage.setItem('id',document.getElementById('submit_userid').value);
    console.log(id);
    const result = await fetch(`http://localhost:3000/reimbursements/author/userid/${id}`, {
          headers: {
...
async function Update_Reimbursement(event) {
    ...
    let id = document.getElementById('submit_reimbursementid');
