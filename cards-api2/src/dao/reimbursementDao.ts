...
import { convertReimbursement}  from '../utils/reimbursementconverter';
...
export async function findReimbursementById(id: number) {
    ...
    try {
        ...
        const reimdao = result.rows[0];

        return reimdao && convertReimbursement(reimdao);
    } catch (err) {
        console.log(err);
    } finally {
        client && client.release();
    ...
    try {
        ...
        const queryString = `
            INSERT INTO reimbursement (amount, author, resolver, date_submited, date_resolved, description, status_id, type)
                ...
                RETURNING *
        `;
        const result = await client.query(queryString, [reimbursement.amount, reimbursement.author.role, reimbursement.resolver && reimbursement.resolver.role,
